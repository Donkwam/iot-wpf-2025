...
namespace MovieFinder2025.ViewModels
{
    ...
    {
        private readonly IDialogCoordinator dialogsCoordinator;
        ...
        {
            //await this.dialogsCoordinator.ShowMessageAsync(this, "영화검색", MovieName);
            if (string.IsNullOrEmpty(MovieName))
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "영화검색", "영화명을 입력하세요!");
                return;
            }

            var controller = await dialogsCoordinator.ShowProgressAsync(this, "대기중", "검색 중...");
        ...
        {
            string tmdb_apiKey = "02d762db6b4b7e38296980b9c5c4d8f0";    // TMDB에서 신청한 API키
            ...
            // OpentAPI 실행할 웹 객체, WebRequest, WebResponse
            // HttpClient로 대체할 것
            //WebRequest req = null;
            ...
            try
            {
                ...
                SearchResult = $"영어검색 건수 : {response.Total_results}건";
                Common.LOGGER.Info(SearchResult + "검색완료!!");
            }
            catch (Exception ex)
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "예외", ex.Message);
        ...
        public async Task MovieItemDoubleClick()
        {
            ...
            if (currMovie != null)
            {
                ...
                await this.dialogsCoordinator.ShowMessageAsync(this, currMovie.Title, sb.ToString());
            }
        }
        ...
        {
            //await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기 추가", "");
            if (SelectedMovieItem == null)
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기 추가", "추가할 영화를 선택하세요.");
                return;
            }
            ...
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    if (resultCnt > 0)
                    {
                        ...
                        await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기추가", "즐겨찾기 추가 성공");
                    }
                    else
                    {
                        await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기추가", "즐겨찾기 추가 실패!");
                    }
                }
            }
            catch (Exception ex)
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "오류", ex.Message);
                Common.LOGGER.Fatal(ex.Message);
            }
        ...
        public async Task DelFavoriteMovies()
        {
            if (SelectedMovieItem == null)
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기삭제", "삭제할 영화를 선택하세요.");
                return;
            }
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    if (resultCnt > 0) 
                    {
                        await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기삭제", "즐겨찾기 삭제 성공");
                    }
                    else
                    {
                        await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기삭제", "즐겨찾기 삭제 실패!");
                    }
                }
            }
            catch (MySqlException ex)
            {
                if (ex.Message.ToUpper().Contains("DUPLCATE ENTRY"))
                {
                    Common.LOGGER.Info($"{SelectedMovieItem.Title} 기추가된 영화");
                    await this.dialogsCoordinator.ShowMessageAsync(this, "즐겨찾기추가", "이미 추가된 즐겨찾기입니다.");
                }
                else
                {
                    await this.dialogsCoordinator.ShowMessageAsync(this, "오류", ex.Message);
        ...
        public async Task ViewMovieTrailer()
        {
            if (SelectedMovieItem == null)
            {
                await this.dialogsCoordinator.ShowMessageAsync(this, "예고편보기", "영화를 선택하세요.");
                return;
            }
